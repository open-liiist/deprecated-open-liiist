//web-client/src/app/%5Blocale%5D/%28login%29/_login.tsx
'use client';
...
export function Login({ mode = 'signin' }: { mode?: 'signin' | 'signup' }) {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const payload = mode === 'signin'
            ? { email, password }
            : { email, password, name, dateOfBirth, supermarkets: selectedSupermarkets };

        const apiUrl = `/auth/${mode === 'signin' ? 'login' : 'register'}`;
