...
import json
sys.path.append('../')
from scraping_dem_shop import scraping_shop
sys.path.append('../../')
from libft import fetch_data, extract_micro_categories
# from send_data import send_data_to_receiver, get_all_stores

if __name__ == "__main__":
	categories_url = "https://www.supermercatidemspesaadomicilio.com/ebsn/api/category?filtered=false&hash=w0d0t0"
	headers = {"Accept": "*/*"}
	...
	for category in micro_categories:
		...
		while True:
			products_url = f"https://www.supermercatidemspesaadomicilio.com/ebsn/api/products?parent_category_id={category_id}&page={page}&page_size=24"
		...
		for product in products:
			try:
				...
				"img_url": f"{'https://www.supermercatidemspesaadomicilio.com'}{product['mediaURLMedium']}",
	...
	for shop in shop_info_list:
		...
		# with open(f"store_dem_{sanitized_street}.json", "w", encoding="utf-8") as outfile:
