...
sys.path.append('../')
from libft_gros import get_html_from_url, has_phone_number, has_superficie, geocode, has_questo_indirizzo
# from send_data import create_store

# Function to scrape shop data from a given URL and return a list with all the information

def scraping_shop():
	generic_url = "https://www.cedigros.com/insegne/itemlist/filter.html?category=51&moduleId=219&Itemid=701&f63b37307b7737ac4a3515d3d0455523=1&format=raw"
	headers = {"Accept": "*/*"}

	response = get_html_from_url(generic_url, headers = headers)
	shop_info_list = []
	shop_name = "idromarket"
	...
	for url in url_shop_list_unique:
		...
		try:
			...
			for div_call in div_info:
				if not (has_phone_number((div_call).get_text()) | has_superficie((div_call).get_text()) | has_questo_indirizzo((div_call).get_text())):
	...
	# with open(f"store_idromarket.json", "w", encoding="utf-8") as outfile:
